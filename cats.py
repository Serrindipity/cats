...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    newList = [x for x in paragraphs if (select(x) == True)]
    if k >= len(newList):
        return ''
    else:
        return newList[k]
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def select(item):
        item = split(lower(remove_punctuation(item)))
        for i in subject:
            if i in item:
                return True
        return False
...
def minimum_mewtations(typed, source, limit):
    ...
    typedIndex = typed.find(source)
    sourceIndex = source.find(typed)
    if typedIndex != -1 :
        return len( typed[ :typedIndex] + typed[typedIndex + len(source): ] )
    
    if sourceIndex != -1:
        return len( source[ : sourceIndex] + source[sourceIndex + len(typed): ])
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i) < min:
                pos_of_fastest = j
                min = time(match, j, i)
        lst[pos_of_fastest].append(get_word(match, i))
    return lst
    # END PROBLEM 10
